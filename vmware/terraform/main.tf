...
module "config_inventory_single" {
  ...
  vm_os_password       = "${var.vm_os_password}"
  domain_name                = "${var.vm_domain_name}"
  single_vm_hostname          = "${element(keys(var.single_node_hostname_ip), 0)}"
  single_vm_ipv4_address      = "${element(values(var.single_node_hostname_ip), 0)}"
  rh_user                    = "${var.rh_user}"
  rh_password                = "${var.rh_password}"
...
module "generate_kubeconfig" {
  ...
  openshift_server    = "${element(keys(var.single_node_hostname_ip),0)}.${var.vm_domain_name}"
  openshift_port      = "8443"
  openshift_user      = "${var.openshift_user}"
