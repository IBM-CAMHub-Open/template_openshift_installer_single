...
module "deployVM_single" {
  ...
  count = "${length(keys(var.ocp_node_hostname_ip))}"

  #######
  // vm_folder = "${module.createFolder.folderPath}"
  enable_vm               = "true"
  vm_vcpu                 = "${var.ocp_node_vcpu}"                                                                                                           // vm_number_of_vcpu
  vm_name                 = "${keys(var.ocp_node_hostname_ip)}"
  vm_memory               = "${var.ocp_node_memory}"
  ...
  vm_ipv4_address         = "${values(var.ocp_node_hostname_ip)}"
...
module "config_inventory_single" {
  ...
  vm_os_password       = "${var.vm_os_password}"
  domain_name                = "${var.vm_domain_name}"
  single_vm_hostname          = "${element(keys(var.ocp_node_hostname_ip), 0)}"
  single_vm_ipv4_address      = "${element(values(var.ocp_node_hostname_ip), 0)}"
  rh_user                    = "${var.rh_user}"
  rh_password                = "${var.rh_password}"
...
module "run_installer" {
  ...
  vm_os_user           = "${var.vm_os_user}"
  vm_os_password       = "${var.vm_os_password}"
  master_vm_ipv4_address     = "${element(values(var.ocp_node_hostname_ip), 0)}"
...
module "generate_kubeconfig" {
  ...
  vm_os_user          = "${var.vm_os_user}"
  vm_os_password      = "${var.vm_os_password}"
  vm_os_private_key   = "${var.vm_os_private_ssh_key}"
  master_node_ip      = "${element(values(var.ocp_node_hostname_ip), 0)}"
